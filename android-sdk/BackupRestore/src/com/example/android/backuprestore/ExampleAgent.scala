...
import java.io.{ByteArrayInputStream, ByteArrayOutputStream,
                DataInputStream, DataOutputStream, File, FileInputStream,
                FileOutputStream, IOException, RandomAccessFile}
 ...
 */
class ExampleAgent extends BackupAgent {
  import ExampleAgent._  // companion object
  ...
  override def onBackup(oldState: ParcelFileDescriptor, data: BackupDataOutput,
    ...
    val doBackup = (oldState == null) || compareStateFile(oldState)
    ...
    if (doBackup) {
      ...
      val outWriter = new DataOutputStream(bufStream)
  ...
  private def compareStateFile(oldState: ParcelFileDescriptor): Boolean = {
    ...
    try {
      ...
      if (stateVersion > AGENT_VERSION) {
        ...
        val lastFilling = in.readInt()
        val lastMayo = in.readBoolean()
        val lastTomato = in.readBoolean()

        (lastFilling != mFilling) ||
  ...
  override def onRestore(data: BackupDataInput, appVersionCode: Int,
    ...
    while (data.readNextHeader()) {
      ...
      if (APP_DATA_KEY equals key) {
        // It's our saved data, a flattened chunk of data all in one buffer.
        // Use some handy structured I/O classes to extract it.
        val dataBuf = new Array[Byte](dataSize)
        data.readEntityData(dataBuf, 0, dataSize)
        val baStream = new ByteArrayInputStream(dataBuf)
        val in = new DataInputStream(baStream)
...
object ExampleAgent {
   ...
   */
  final val APP_DATA_KEY = "alldata"
