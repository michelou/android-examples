 ...
 *
 */
class KubeRenderer(world: GLWorld, callback: AnimationCallback)
extends AnyRef with GLSurfaceView.Renderer {

  def onDrawFrame(gl: GL10) {
    if (callback != null) callback.animate()

    /*
     * Usually, the first thing one might want to do is to clear the screen.
     * The most efficient way of doing this is to use glClear(). However we
     * must make sure to set the scissor correctly first. The scissor is always
  ...
  def setAngle(angle: Float) { mAngle = angle }

  def getAngle: Float = mAngle
