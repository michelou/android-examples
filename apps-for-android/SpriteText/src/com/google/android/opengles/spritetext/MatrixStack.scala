...
class MatrixStack(maxDepth: Int) {
  ...
  private val mMatrix = new Array[Float](maxDepth * MATRIX_SIZE)
  private val mTemp = new Array[Float](MATRIX_SIZE * 2)
  ...
  def glOrthof(left: Float, right: Float, bottom: Float,
  ...
  private def fixedToFloat(x: Int): Float = x * (1.0f / 65536.0f)
