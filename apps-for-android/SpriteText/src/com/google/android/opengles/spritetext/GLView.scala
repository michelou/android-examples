...
import javax.microedition.khronos.egl.{EGL10, EGL11, EGLConfig}
import javax.microedition.khronos.egl.{EGLContext, EGLDisplay, EGLSurface}
 ...
 * instance.
 */
class GLView(context: Context, attrs: AttributeSet) extends SurfaceView(context, attrs)
  ...
  class GLThread(renderer: Renderer) extends Thread {
    ...
    def onWindowFocusChanged(hasFocus: Boolean) {
      synchronized {
        mHasFocus = hasFocus
        if (mHasFocus) notify()
      }
    ...
    def onWindowResize(w: Int, h: Int) {
      synchronized {
        mWidth = w
        mHeight = h
        mSizeChanged = true
    ...
    private var mEventQueue = new ArrayList[Runnable]()
