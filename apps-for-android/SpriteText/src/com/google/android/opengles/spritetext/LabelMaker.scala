...
class LabelMaker(fullColor: Boolean, strikeWidth: Int, strikeHeight: Int) {
   ...
   */
  private val mFullColor = fullColor
  private val mStrikeWidth = strikeWidth
  private val mStrikeHeight = strikeHeight
  private val mTexelWidth = (1.0 / mStrikeWidth).toFloat  // Convert texel to U
  private val mTexelHeight = (1.0 / mStrikeHeight).toFloat // Convert texel to V
  private val mClearPaint = new Paint()
  ...
  def add(gl: GL10, background: Drawable, text: String, textPaint: Paint,
    ...
    // Make changes to the local variables, only commit them to the
    // member variables after we've decided not to throw any exceptions.
   ...
   */
  def getWidth(labelID: Int): Float = mLabels.get(labelID).width
   ...
   */
  def getHeight(labelID: Int): Float = mLabels.get(labelID).height
   ...
   */
  def getBaseline(labelID: Int): Float = mLabels.get(labelID).baseline
  ...
  def draw(gl: GL10, x: Float, y: Float, labelID: Int) {
    ...
    val label = mLabels.get(labelID)
    gl glEnable GL10.GL_TEXTURE_2D
    gl.asInstanceOf[GL11].glTexParameteriv(GL10.GL_TEXTURE_2D,
                GL11Ext.GL_TEXTURE_CROP_RECT_OES, label.crop, 0)
    gl.asInstanceOf[GL11Ext].glDrawTexiOES(
      snappedX.toInt, snappedY.toInt, 0,
  ...
  private var mLabels = new ArrayList[Label]()
...
object LabelMaker {
  ...
  private class Label(val grid: Grid, val width: Float, val height: Float,
                      val baseline: Float,
                      cropU: Int, cropV: Int, cropW: Int, cropH: Int) {
    val crop = Array(cropU, cropV, cropW, cropH)
  }
}
