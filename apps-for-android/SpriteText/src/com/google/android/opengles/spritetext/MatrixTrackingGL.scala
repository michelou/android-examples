...
import java.nio.{Buffer, ByteBuffer, ByteOrder, FloatBuffer, IntBuffer, ShortBuffer}

import javax.microedition.khronos.opengles.{GL, GL10, GL10Ext, GL11, GL11Ext}

import javax.microedition.khronos.opengles.GL10._
 ...
 * driver.
 */
class MatrixTrackingGL(gl: GL) extends AnyRef with GL
                                              with GL10 with GL10Ext
                                              with GL11 with GL11Ext {
  import MatrixTrackingGL._  // companion object

  private val mgl: GL10 =
    if (gl.isInstanceOf[GL10]) gl.asInstanceOf[GL10] else null
  private val mgl10Ext: GL10Ext =
    if (gl.isInstanceOf[GL10Ext]) gl.asInstanceOf[GL10Ext] else null
  private val mgl11: GL11 =
    if (gl.isInstanceOf[GL11]) gl.asInstanceOf[GL11] else null
  private val mgl11Ext: GL11Ext =
    if (gl.isInstanceOf[GL11Ext]) gl.asInstanceOf[GL11Ext] else null

  private var mMatrixMode: Int = GL10.GL_MODELVIEW
  private var mModelView: MatrixStack = new MatrixStack()
  private var mTexture: MatrixStack = new MatrixStack()
  private var mProjection: MatrixStack = new MatrixStack()
  private var mCurrent: MatrixStack = mModelView
  ...
  def glColorPointer(size: Int, typ: Int, stride: Int, pointer: Buffer) {
    mgl.glColorPointer(size, typ, stride, pointer)
  }
  ...
  def glCopyTexSubImage2D(target: Int, level: Int, xoffset: Int,
  ...
  def glDrawElements(mode: Int, count: Int, typ: Int, indices: Buffer) {
    mgl.glDrawElements(mode, count, typ, indices)
  }
  ...
  def glFrustumx(left: Int, right: Int, bottom: Int, top: Int, near: Int,
                 far: Int) {
  ...
  def glMatrixMode(mode: Int) {
    mCurrent = mode match {
      case GL10.GL_MODELVIEW  => mModelView
      case GL10.GL_TEXTURE    => mTexture
      case GL10.GL_PROJECTION => mProjection
      case _ => throw new IllegalArgumentException("Unknown matrix mode: " + mode)
    }
  ...
  def glNormalPointer(typ: Int, stride: Int, pointer: Buffer) {
    mgl.glNormalPointer(typ, stride, pointer)
  }
  ...
  def glReadPixels(x: Int, y: Int, width: Int, height: Int, format: Int,
                   typ: Int, pixels: Buffer) {
    mgl.glReadPixels(x, y, width, height, format, typ, pixels)
  }
  ...
  def glTexCoordPointer(size: Int, typ: Int, stride: Int, pointer: Buffer) {
    mgl.glTexCoordPointer(size, typ, stride, pointer)
  }
  ...
  def glTexImage2D(target: Int, level: Int, internalformat: Int,
                   width: Int, height: Int, border: Int, format: Int, typ: Int,
                   pixels: Buffer) {
  ...
  def glTexSubImage2D(target: Int, level: Int, xoffset: Int,
                      yoffset: Int, width: Int, height: Int,
                      format: Int, typ: Int, pixels: Buffer) {
  ...
  def glVertexPointer(size: Int, typ: Int, stride: Int, pointer: Buffer) {
    mgl.glVertexPointer(size, typ, stride, pointer);
  }
  ...
  def glPointSizePointerOES(typ: Int, stride: Int, pointer: Buffer) {
    throw new UnsupportedOperationException()
  }
  ...
  def glColorPointer(size: Int, typ: Int, stride: Int, offset: Int) {
    throw new UnsupportedOperationException()
  }

  def glDrawElements(mode: Int, count: Int, typ: Int, offset: Int) {
    throw new UnsupportedOperationException()
  }
  ...
  def glNormalPointer(typ: Int, stride: Int, offset: Int) {
    throw new UnsupportedOperationException()
  }

  def glTexCoordPointer(size: Int, typ: Int, stride: Int, offset: Int) {
    throw new UnsupportedOperationException()
  }

  def glVertexPointer(size: Int, typ: Int, stride: Int, offset: Int) {
    throw new UnsupportedOperationException()
  }
  ...
  def glMatrixIndexPointerOES(size: Int, typ: Int, stride: Int, pointer: Buffer) {
    throw new UnsupportedOperationException()
  }

  def glMatrixIndexPointerOES(size: Int, typ: Int, stride: Int, offset: Int) {
    throw new UnsupportedOperationException()
  }

  def glWeightPointerOES(size: Int, typ: Int, stride: Int, pointer: Buffer) {
    throw new UnsupportedOperationException()
  }

  def glWeightPointerOES(size: Int, typ: Int, stride: Int, offset: Int) {
    throw new UnsupportedOperationException()
  }
  ...
  private def check() {
    ...
    var fail = false
    for (i <- 0 until 16 if mCheckA(i) != mCheckB(i)) {
