...
import _root_.android.widget.{Button, EditText, ImageButton, ImageSwitcher,
                              ListView, Spinner, TabHost, TableLayout,
...
package object android {
  ...
  private[scala] trait FindView[T <: { def findViewById(id: Int): View }] {
    ...
    @inline def findImageSwitcher(id: Int): ImageSwitcher =
      source.findViewById(id).asInstanceOf[ImageSwitcher]
    ...
    @inline def findTextSwitcher(id: Int): TextSwitcher =
      source.findViewById(id).asInstanceOf[TextSwitcher]
  ...
  implicit def findViewInView[V <: View](view: V) = new FindView[V]() {
    ...
    @inline def findImageSwitcherWithTag(tag: AnyRef): ImageSwitcher =
      source.findViewWithTag(tag).asInstanceOf[ImageSwitcher]
    ...
    @inline def findTextSwitcherWithTag(tag: AnyRef): TextSwitcher =
      source.findViewWithTag(tag).asInstanceOf[TextSwitcher]
