...
object GLCube {
  ...
  private val one = 65536

  private final val mVertexBuffer: IntBuffer = {
    val half = one / 2
    val vertices = Array(
      // FRONT
    ...
    val vertexBuffer = vbb.asIntBuffer()
    vertexBuffer put vertices
    vertexBuffer position 0
    vertexBuffer
  }

  private final val mTextureBuffer: IntBuffer = {
    val texCoords = Array(
      // FRONT
    ...
    val textureBuffer = tbb.asIntBuffer()
  ...
  def loadTexture(gl: GL10, context: Context, resource: Int) {
    val bmp = BitmapFactory.decodeResource(context.getResources, resource)
